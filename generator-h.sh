...
FAMILYNAME="$2"
DISP_FAMILYNAME="$3"
DISP_FAMILYNAME_JP="$DISP_FAMILYNAME"
...
GEN_SCRIPT_UDEAWN='gen_script_udeawn.pe'

# BIZ UDゴシックの調整
cat > "${WORK_DIR}/${GEN_SCRIPT_UDEAWN}" << _EOT_
...
while (i < SizeOf(input_list))
  ...
  while (j < array_end)
    ...
    if (WorthOutputting(ucode))
      Select(ucode)
      w = GlyphInfo("Width")
      if (w > 0 && w > ${ORIG_HALF_WIDTH})
        Move(-${ORIG_HALF_WIDTH}, 0)
  ...
  SetTTFName(0x409, 1, "$DISP_FAMILYNAME")
  SetTTFName(0x409, 2, fontstyle_list[i])
  SetTTFName(0x409, 3, "$DISP_FAMILYNAME" + " : " + Strftime("%d-%m-%Y", 0))
  SetTTFName(0x409, 4, "$DISP_FAMILYNAME" + " " + fontstyle_list[i])
  ...
  SetTTFName(0x411, 3, "$DISP_FAMILYNAME_JP" + " : " + Strftime("%d-%m-%Y", 0))
  SetTTFName(0x411, 4, "$DISP_FAMILYNAME_JP" + " " + fontstyle_list[i])
  SetTTFName(0x411, 5, version)
  SetTTFName(0x411, 6, fontfamily + "-" + fontname_style)
...
fontforge -script ${WORK_DIR}/${GEN_SCRIPT_UDEAWN}

...
# Ambiguousでないが、Illusion-Nに含まれて、BIZ UDゴシックに含まれないグリフ
# grep '^N' onlyhasillu.txt | cut -d ' ' -f2 | sed 's/u/x/'
onlyhasillu = [ \\
  0x2000, 0x2001, 0x2002, 0x2003, 0x2004, 0x2005, 0x2006, 0x2007, 0x2008, \\
  0x2009, 0x200A, 0x2012, 0x2017, 0x201B, 0x201F, 0x2023, 0x2031, 0x2034, \\
  0x2036, 0x2037, 0x2038, 0x203D, 0x2040, 0x2043, 0x2045, 0x2046, 0x2050, \\
  0x20DD, 0x254C, 0x254D, 0x254E, 0x254F, 0x2574, 0x2575, 0x2576, 0x2577, \\
  0x2578, 0x2579, 0x257A, 0x257B, 0x257C, 0x257D, 0x257E, 0x257F, 0x2590, \\
  0x2591, 0x2596, 0x2597, 0x2598, 0x2599, 0x259A, 0x259B, 0x259C, 0x259D, \\
  0x259E, 0x259F, 0x2630, 0x2631, 0x2632, 0x2633, 0x2634, 0x2635, 0x2636, \\
  0x2637]
# 以下のPowerline Symbolsも該当するが、ひとまず対象外にする
...
while (i < SizeOf(input_list))
  ...
  endloop
  # Ambiguousでないが、Illusion-Nに含まれて、BIZ UDゴシックに含まれないグリフ
  onlyhasillu_end = SizeOf(onlyhasillu)
  j = 0
  while (j < onlyhasillu_end)
    ucode = onlyhasillu[j]
    if (WorthOutputting(ucode))
      SelectMore(ucode)
    endif
    j++
  endloop
  # UDEAWNで使うグリフのみを残して削除。BIZ UDゴシック側を置換しないように。
  SelectInvert(); Clear()
